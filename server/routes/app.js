...
async function openShift({ body, session }) {
  ...
  // THE VERIFICATION GATE (decision-47), on the OPEN half and nowhere else. Applied HERE
  // rather than inside the resolver so that "nobody has proved this card yet" keeps its own
  // name instead of collapsing into `unknown_location`. `activePlace` no longer has a
  // building branch to bypass this with (decision-69): every place reaching this line
  // carries a real zone, so the gate is now truly unconditional on what it checks.
