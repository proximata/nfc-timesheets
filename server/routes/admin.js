...
async function adminData({ query }) {
  ...
  const [
    ...
    // LEFT JOIN, not INNER (decision-54 §1): since migration 013 a zone may have NO building
    // at all — an operator wrote a card at a door and deferred the "which object is this?"
    // question. An INNER JOIN would silently DROP exactly those rows, so the one screen an
    // admin has for "what zones exist" would hide the newest ones and nothing would say so.
    // `l.name` comes back NULL for them and the panel renders that read-only: an admin can no
    // longer create or bind a zone, that is the operator's job now (decision-54 §2/§3).
    // ORDER BY l.name puts them LAST — Postgres sorts NULLS LAST for ASC — which is the right
    // end of the list for rows that belong to no building yet.
    all(
...
/**
 * POST /admin/zones -> UPDATE ONLY. An `id` is now mandatory; a body without one is refused.
 *
 * IT USED TO CREATE TOO, and decision-54 §2 took exactly that half away: a zone is born in
 * the field, at the door, in the operator's hand (POST /operator/tags/:id/resolve-zone), and
 * nowhere else — "one place a zone is ever born" is the whole point, and a second creation
 * surface at a desk would re-open the case the decision closed. EDITING an existing zone
 * (name, note, area, serial, active) stays the DIRECTOR'S job and is untouched below.
 *
 * REFUSED, NOT 404'd, when `id` is absent: the route still exists and still works, so a 404
 * would be a lie about the URL rather than an answer about the request. 410 says "this used
 * to be here and is deliberately gone", and the named code tells the panel where it went.
 *
 * 410 zone_creation_moved_to_operator_app  no `id` — create it from the operator app instead
...
async function upsertZone({ body }) {
  ...
  const active = v.bool(body.active, "active", true);
  const targetId = body.id === undefined || body.id === null ? null : v.uuid(body.id, "id");
  // decision-54 §2. Checked BEFORE the serial lookup below so a create attempt costs one
  // branch and no query, and so the answer can never be a 409 about a serial on a request
  // that was never going to be honoured anyway.
  if (targetId === null) fail(410, "zone_creation_moved_to_operator_app");
...
// reported card then becomes that building's FIRST ZONE — through the OPERATOR's
// POST /operator/tags/:id/resolve-zone since decision-54 §2, which deleted the admin route
// this line used to point at (see the block below).
//
...
// *** POST /admin/tags/:id/resolve-zone IS DELETED (decision-54 §2). ***
//
// It turned a freshly field-written card into a zone AT A DESK, days after the operator who
// wrote it stood in front of the door. That operator is the only person who knows which door
// it is, and — the half that made this decision urgent — is often the only one who does NOT
// yet know which building the door belongs to. This route could not express that: it demanded
// a `location_id`, because until migration 013 a zone with no building was not a row.
//
// WHAT REPLACES IT: POST /operator/tags/:id/resolve-zone {name, location_id?}, auth
// "operator" (routes/operator.js). Same CTE, same 404/409 vocabulary, same `tag_deployed_at`
// taken from the REPORT — with `location_id` OPTIONAL, so the card can become a zone now and
// get a building later through POST /operator/zones/:id/bind.
//
// NOT KEPT IMPORTABLE, for the same reason decision-47 gave a few lines above: a retired
// handler that still compiles is one a later reader finds, assumes is supported, and re-wires
// — and re-wiring a route that mints zone ids is invisible until a card is on a wall, and
// then it is permanent.
//
// EDITING a zone is NOT retired with it: POST /admin/zones still updates name, note, area,
// serial and active. Only BIRTH moved.
//
// Pinned by a check: POST /admin/tags/<id>/resolve-zone answers 404, and its RED case is
// putting the route-table entry back.
