...
try {
  ...
  // ===================================================================================
  {
    ...
    // ---- RESOLVE: minting a NEW building-level tag is RETIRED (decision-47) -----------
    await test("resolve-building is GONE — a card can no longer become a building's own tap surface", async () => {
      // RED: put the route-table entry back in adminRoutes -> this answers 201 and a card is
      // a direct-tap BUILDING surface for ever, which is the exact thing being retired.
      const tagId = uuid(95);
      await expect(await reportTag(tagId, op1.cookie), 201);

      const gone = await asAdmin(`/admin/tags/${tagId}/resolve-building`, {
        method: "POST",
      ...
      assert.equal(
        ...
        0,
        "nothing may have been created on the way to that 404",
      );
      assert.equal(
        (await admin.query("SELECT resolved_at FROM reported_tags WHERE id = $1", [tagId])).rows[0].resolved_at,
        null,
        "...and the card is still on the admin's worklist, waiting for a real decision",
      );

      // Not "unrouted but importable" either: the handler is DELETED, so a later reader
      // cannot find it, assume it is supported and wire it back up.
      const adminSource = readFileSync(new URL("./routes/admin.js", import.meta.url), "utf8");
      assert.ok(
        !adminSource.includes("async function resolveTagToBuilding"),
        "the handler must be DELETED, not kept importable — the id space is shared, so a silent re-wire is permanent the moment a card is mounted",
      );
      const { adminRoutes } = await import("./routes/admin.js");
      assert.equal(
        adminRoutes.filter((r) => r.path.includes("resolve-building")).length,
        0,
        "no route-table entry may match resolve-building",
      );

      // THE REPLACEMENT, end to end, in the same test: a building is created TAG-FREE (its
      // id comes from the DATABASE, never from the caller), and the reported card becomes
      // that building's FIRST ZONE — unverified, exactly like every other zone.
      const house = await expect(
        await asAdmin("/admin/locations", { method: "POST", body: { slug: "tag-free-haus", name: "Neu entdecktes Haus" } }),
        201,
      );
      ...
      assert.equal(firstZone.zone.verified_at, null, "and it is not tappable until an operator proves it in the field");
    });
    ...
    await test("GET /admin/data lists UNRESOLVED reported tags, and drops them the moment they resolve", async () => {
      ...
      const worklistHouse = await expect(
        await asAdmin("/admin/locations", { method: "POST", body: { slug: "worklist-haus", name: "Wartelistenhaus" } }),
        201,
      );
      await expect(
        await asAdmin(`/admin/tags/${tagId}/resolve-zone`, {
