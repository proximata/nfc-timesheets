...
| 3 | **TASK-254** — operator-only Android phones have no self-update path | A phone used only for mounting cards can never receive a fix by itself; someone has to drive out and sideload it. Already bit us same-day. | O · confirmed still accurate, zero `UpdateManager` refs in either operator screen |
