...
| # | Feature | iOS | Android | Verdict |
|---|---|---|---|---|
| 30 | Self-update from inside the app | impossible | **REMOVED 2026-08-26** — `UpdateManager.kt`/`GET /app/version`/`/app/download` deleted once Android distribution moved to the Play Store; Play's own updater now reaches every device, so neither platform self-updates | **N/A-PLATFORM, both sides now** — App Store guideline 2.5.2 already banned it on iOS; Android dropped its counterpart once store distribution was live. iOS's passive sentence at `API.swift:189` ("This app version was rejected by the server. Update it.") is unchanged. TASK-52/TASK-53 research closed Wont Do as moot |
| 31 | Version check against the server (`GET /app/version`) | **not called at all** | **REMOVED 2026-08-26** — the route this checked no longer exists on either platform | **N/A, both sides** — store-delivered updates make an in-app version check unnecessary; superseded by decision-52's plain version LINE (no server call, no update logic) shown on both platforms' Settings screens |
