...
// database), and the reported card then becomes that building's FIRST ZONE via
// `resolveTagToZone`. Kept as a comment and not as a dead function on purpose: a client
// helper that still compiles is one a later screen calls, and the 404 would only surface at
// runtime, on an admin's screen, mid-field-visit.
